#!/usr/bin/env python3
"""
Terraform Manager MCP - HTTP API Server

This service provides an HTTP API for the Terraform Manager MCP,
...
from src.mcp import TerraformManagerMCP


# Global MCP instance
_mcp: TerraformManagerMCP = None
...
async def execute_handler(request):
    ...
    POST /execute
    {
        ...
        "parameters": {"role": "mim", "domain": "aptus2", "operation": "add", "count": 2},
        "user_id": "U123",
        "channel_id": "C456"
    }
    """
...
async def init_app():
    ...
    logger.info("Initializing Terraform Manager MCP...")
    _mcp = TerraformManagerMCP()
...
def main():
    """Main entry point."""
    print("=" * 60)
    print("Terraform Manager MCP")
    print("=" * 60)

    port = int(os.environ.get("PORT", 8083))

    logger.info(f"Starting Terraform Manager MCP on port {port}...")
